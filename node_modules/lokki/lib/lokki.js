...
// Others
var resource_urls = ["index","show","new","create","edit","update","destroy"];
...
LokkiFramework.prototype.init = function() {
  console.log("boot lokki mvc"); 
  // Boot Libraries
  this._libs = bootLibs();
  ...
  GLOBAL[this._app_name] = {lib: this._libs, routes:methods, middleware: {}};
  // Boot Controllers
...
var methods = {
  ...
  resources: function(controller_name,params) {
    ...
    Object.keys(actions).map(function(action) {
      ...
      if(resource_urls.indexOf(action) != -1) {
        var fn = controllerAction(controller_name,controller_name_plural,action,actions[action].fn);
...
function generate_routes(action, methods, prefix, before_filter, fn) {
  switch(action) {
    case 'index':
      App.get(prefix, before_filter, fn);
      break;
    case 'show':
      App.get(prefix + '/:id',before_filter, fn);
      break;
    case 'add':
      App.get(prefix + '/:id/add',before_filter, fn);
      break;
    case 'create':
      App.post(prefix + '/:id',before_filter, fn);
      break;
    case 'edit':
...
function bootController(controller) {
  ...
  Object.keys(controller_actions).map(function(action) {
    ...
    // Run across every Before Filter
    before_filters.forEach(function(filter) {
      var filter_fn = filter[0],
          args = filter[1] || {},
...
var initApp = function() {
  ...
  return function(name, express, app) {
